...
def update_pet_stats(user_id):
    ...
    # Уменьшаем показатели (по 2 единицы в час для более мягкого понижения)
...
def get_random_phrase(pet_type, action="talk"):
    ...
    # Фразы по умолчанию если что-то пошло не так
    default_phrases = {
...
async def pet_action(callback: types.CallbackQuery):
    ...
    if action == "feed":
        ...
        # Сон: восстанавливает энергию, немного поднимает настроение
...
async def talk_to_pet(message: types.Message):
    ...
    # Разговор немного тратит энергию (как умственная активность)
...
async def absurd_debates(message: types.Message):
    ...
    # Дебаты сильно тратят энергию (мозговая активность), но поднимают настроение
