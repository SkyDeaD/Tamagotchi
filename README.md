# README: Telegram-бот Tamagotchi с игрой в города
...
2. **Настройка токена**:
   ...
   - **Альтернативный способ**: Укажите токен напрямую в коде, заменив:
     ...
     BOT_TOKEN = "ваш_токен"
     ```
     **Внимание**: Указание токена в коде небезопасно, так как он может быть случайно опубликован (например, в репозитории GitHub). Всегда используйте переменные окружения для продакшн-версий.
